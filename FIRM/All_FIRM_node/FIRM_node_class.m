classdef FIRM_node_class
    ...
    properties (Constant)
        ...
        GHb_neighb_magnifyikng_coeff = 35;
    end
    ...
    methods
        function obj = FIRM_node_class( node_center_inp )
            if nargin>0
                ...
                GHb_conv_reg_thresh = state.mean_neighborhood_size*GHb_magnifying_coeff; 
                
                BigX_thresh = [GHb_conv_reg_thresh;GHb_conv_reg_thresh];
                obj.par.GHb_conv_BigX_thresh = BigX_thresh; % distance threshold for both Xg_mean and Xest_mean_mean in the single vector
                obj.par.GHb_conv_Pest_thresh = GHb_conv_reg_thresh*GHb_conv_reg_thresh'; % defines the convergence threshold for Pest
                obj.par.GHb_conv_BigCov_thresh = BigX_thresh*BigX_thresh'; % defines the convergence threshold for BigCov
            end
        end
        ...
        function YesNo = is_reached(obj,b)
            ...
            % size of the region:
            X_reg_size = state.mean_neighborhood_size*mean_neighb_magnifying_coeff; % read the comment after the "obj.par" initialization, to see why I have written the current line.
            Pest_reg_size = belief.cov_neighborhood_size*cov_neighb_magnifying_coeff;
