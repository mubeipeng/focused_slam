classdef obstacles_class
    ...
    methods
        function obj = obstacles_class(obstacleStructure)
            obj.obst = obstacleStructure.obst;
            obj.boundary_handle = obstacleStructure.boundary_handle;
            obj.fill_handle = obstacleStructure.fill_handle;
        end
    end
    ...
    methods (Static = true)
        ...
        function obstacleMap = get_obstacles(sim)
            global par;
            Man_Obst = sim.par.intractive_obst;
            LoadFileName = par.environmentFile;
            if Man_Obst == 0
                if exist(LoadFileName,'file')
                    obstacleMap = obstacles_class.load(sim,LoadFileName);
            ...
            if Man_Obst == 1
                obstacleMap = obstacles_class.request(sim, LoadFileName);
                Obst_vertices = obstacleMap.obst; %#ok<NASGU>
            end
        end
        function tmp_prop = load(sim, LoadFileName)
            ...
            for idx_obj =1:numel(objOutput.objects)
                ...
                elseif strcmp(objOutput.objects(idx_obj).type,'f')
                    ...
                    if isempty(h_obs) && ib==0
                        ...
                        fill_color_handle = fill(inputed_obstacles{ib}(:,1),inputed_obstacles{ib}(:,2),'k');
        ...
        function tmp_prop = request(sim, fileName)
            ...
            figure(gcf);
            axis(gca, sim.par.env_limits)
            title({'Please mark the vertices of polygonal obstacles','Press ESC when done'},'fontsize',14)
            ib=0;
            tmp_prop.boundary_handle = [];
            tmp_prop.fill_handle = [];
            while true
        ...
        function plot_handle = draw()
            ...
            for ob_ctr = 1:length(obstacles_class.obst)
                if 1%obstacles_class.plot_3D_flag== 1  % simulator has the 3D plotting flag. you can input it if needed.
