classdef Quadrotor_state < state_interface
    ...
    properties (Constant)
        ...
        % Note that the order of variables is 
        % [x,y,z, x_dot, y_dot, z_dot, roll, pitch, yaw, roll_dot,
        % pitch_dot, yaw_dot]
    end
    properties
    ...
    methods
        ...
        function signed_dist_vector = signed_element_wise_dist(obj,x2) % this function returns the "Signed element-wise distance" between two states x1 and x2
            ...
            for i = 7:9  % note that "roll, pitch, yaw" angles appear at entries 7 to 9, NOT 4 to 6.
       ...
       function obj = draw(obj, varargin)
            ...
            % The "eul2r" is a function Peter Corke's Robotic Toolbox. The function accepts "row vector".
            R = eul2r(obj.val(7:9)');  % note that "roll, pitch, yaw" angles appear at entries 7 to 9, NOT 4 to 6.
            ...
            if ~isempty(robot_text)
                obj.text_handle = text(obj.val(1),obj.val(2),obj.val(3)+5, robot_text,'fontsize',font_size,'color',text_color);
            end
