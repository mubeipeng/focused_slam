classdef Stationary_LQR_class < LQR_interface
    ...
    methods
        function obj = Stationary_LQR_class(original_sys_inp, lnr_sys_inp, lnr_pts_inp)
            obj.Final_state_cost = eye(size(lnr_sys_inp(1).A,1))*user_data_class.par.LQR_cost_coefs(1);
            obj.state_cost = eye(size(lnr_sys_inp(1).A,1))*user_data_class.par.LQR_cost_coefs(2);
            obj.control_cost = eye(size(lnr_sys_inp(1).B,2))*user_data_class.par.LQR_cost_coefs(3);
    ...
    methods (Access = private)
        function feedbak_gain = generate_feedback_gain(obj, lnr_sys_inp)
            W_x=obj.state_cost * user_data_class.par.state_cost_ratio_for_stationary_case; % note that "state_cost" is for the trajectory tracking. Usually in point stabilization, we need more force on state and less on control. So, we multiply the "state_cost" to an appropriate ratio. Note that this ratio has to be greater than 1.
            W_u=obj.control_cost * user_data_class.par.control_cost_ratio_for_stationary_case; % note that "control_cost" is for the trajectory tracking. Usually in point stabilization, we need more force on state and less on control. So, we multiply the "control_cost" to an appropriate ratio. Note that this ratio has to be LESS than 1.
