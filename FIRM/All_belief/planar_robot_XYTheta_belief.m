classdef planar_robot_XYTheta_belief < Gaussian_belief_interface
    ...
    properties (Constant)
        FIRM_node_cov_neighborhood_size = planar_robot_XYTheta_state.FIRM_node_mean_neighborhood_weights*planar_robot_XYTheta_state.FIRM_node_mean_neighborhood_weights'; % this only works for 3D state spaces % note that the last entry, ie theta's neighborhood, has to be in radian. % This is a matrix.
    end
    ...
    methods
        ...
        function obj = draw_CovOnNominal(obj, nominal_state, varargin)
           ...
           if ~isempty(ellipse_spec)
                tmp=get(gca,'NextPlot'); hold on
                obj.ellipse_handle = plotUncertainEllip2D(obj.est_cov(1:2,1:2),nominal_state.val(1:2),ellipse_spec, ellipse_width, ellipse_magnify);
                set(gca,'NextPlot',tmp);
            end
        end
    end
