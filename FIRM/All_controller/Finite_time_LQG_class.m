    ...
    methods
        function obj = Finite_time_LQG_class(system_inp, nominal_trajectory, filter_mode_inp)
            ...
            if nargin > 0
                ...
                obj.separated_controller = Finite_time_LQR_class(system_inp, obj.lnr_sys, obj.lnr_pts);
        ...
        function [nextBelief, sim, logged_data] = executeOneStep(obj,oldBelief,k,sim,noiseFlag)
            ...
            % generating feedback controls
            [u ] = obj.separated_controller.generate_feedback_control(oldBelief,k);
            % Apply control
            ...
            elseif strcmpi(obj.filter_mode,'EKF')
             ...
             logged_data = [];
             if nargout == 4
                 logged_data.k_control = k;
                 logged_data.k_obs = k+1;
                 logged_data.x = sim.robot.ss.val;
                 logged_data.b = sim.robot.belief;
                 logged_data.u = u;
                 logged_data.z = z;
             end
